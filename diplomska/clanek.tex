...
\begin{center}
    ...
    \end{tikzpicture}.
\end{center}
Na levi strani pišemo vhode (v tem primeru \(\ket{a}\)), črte pa predstavljajo kompozicijo oziroma tok podatkov. Tukaj gre bit \(\ket{a}\) v vrata za negacijo. Na desni strani vrat pa dobimo negiran bit. Pogledamo si še malo bolj zapleteno vezje. Tukaj predstavimo De Morganov zakon
\[
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node (a1) at (-1.5,-0.5) {\(\ket{a}\)};
        \node (b1) at (-1.5,0.5) {\(\ket{b}\)};
        \node (out1) at (2,0) {};
